---
layout: project
title: "Project 2: LVar → x86-64 Assembler"
due: 2025-10-09
duedate: 2025-10-09
...
_Starter code_: [Download the starter here!]({{ "/assets/p2-starter.tar.gz" | relative_url }})

<div align="center">
	<img src="{{ '/assets/img/p1_demo.png' | relative_url }}" alt="Debug server demo" />
</div>

(Pictured: `debug-server.rkt` in action.)
...
You will compile a tiny expression language (R1/LVar, used
interchangably) for straight-line arithmetic with `let`-bindings and
`read` into x86-64 assembly, then produce a real binary and test it on
actual inputs. Essentially, programs in our language are sequences of
reads, ultimately printing one final output.
...
- `main.rkt` – Driver that runs all passes, can build a binary
- `debug-server.rkt` – Launches a debugging server to help visualize each compiler pass
...
The autograder tests use the `tester.py` infrastructure. These tests
are designed to be compatible with the Autograder. You can use the
standard `python3 tester -av` to run all tests (in verbose mode), or
`python3 tester.py -t <TEST> -v` to run a specific test.
